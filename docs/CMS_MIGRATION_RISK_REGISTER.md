...
| R-020 | Policy local type 無法對應 Sanity category | Policy | Medium | Medium | dry-run 建立明確 type-to-category mapping，無 mapping 時 blocked；Phase 12C 已新增 `weather` category | `pnpm run migration:dry-run` blocking errors | 不匯入該 Policy，等待分類決策 | Owner confirms category mapping | Mitigated for current local policies |
| R-021 | Draft payload 被誤認為可直接 import | First batch payload | Medium | High | payload 檔放在 ignored `.migration-reports/`；README 明確禁止本階段 import | review report 與 Git staged files 檢查 | 不執行 import；若誤匯入需使用 allowlist rollback plan | Owner confirms before Phase 12D/13 | Open |
